...
export default createStore({
  state: {
      ...
      dropDownSentence: [
        {qid:1, 
          ...
          p1choices: ["choice 1 for first part", "second choice for first part", "third choice for first part"],
          p1correct: 1,
          questionp2: "part 2 question text for multiple choice",
          p2choices: ["choice 1 for 2nd part", "second choice for 2nd part","third choice for 2nd part"],
          p2correct: 3,
  ...
  getters: {
      ...
      questionNext: function(state){
          ...
          var nextQ = eval("state.".concat(quizTitle,"[",state.currentXid-1,"].xid"))
          console.log(nextQ)
          var toRet
          var exe = "for(var i=0; i<state.".concat(tableSearch, ".length; i++){ if(state.", tableSearch, "[i].xid===", nextQ, "){toRet=state.", tableSearch, "[i];} else {toRet=0;}}" )
          eval(exe)
